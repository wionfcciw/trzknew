...
namespace BLL
{
    public class BLL_zxlq_xx
    {
        ...
        public DataTable selectksh(string lqxx)
        {
            try
            {
                string sql = "    SELECT  a.*,c.xm, c.cj, d.zydm+ d.zymc as lqzy  FROM    zk_zxlq a left join zk_lqk c on a.ksh=c.ksh   left join zk_zyk d on a.lqxx=d.xxdm and c.lqzy=d.zydm where a.lqxx=@lqxx and a.types=0 ";
        ...
        public string ImportExcelData(string excelFilePath)
        {
           ...
           // string sqlCmd = "update zk_lqk set  xqdm='500',lqxx=@lqxx,pcdm=@pcdm,zydm='00',lqzy=@lqzy,sf_zbs=0,sf_tfgj=0,td_zt=2,zysx=1,xx_zt=4,types=2 where ksh=@ksh and isnull(td_zt,0)=0";
            string sqlCmd = "insert into zk_zxlq (ksh,lqxx,zxyy,types) values (@ksh,@xxdm,@zxyy,0); ";
            string checkSql = "select * from zk_lqk where ksh=@ksh;select * from zk_zxlq where ksh=@ksh;";
            ...
            try
            {
                foreach (var element in excel)
                {
                    ...
                    List<SqlParameter> param = new List<SqlParameter> {
                                                new SqlParameter("@xxdm",xxdm),
                                                new SqlParameter("@zxyy",element[1].ToString().Trim()),
                    ...
                    {
                        if (tmpDataSet.Tables[0].Rows[0]["lqxx"].ToString() != xxdm)
                        {
                            errMsg += "第" + (i + 1) + "条数据有误，原因是：该考生未录取在您的学校：" + element[0].ToString().Trim() + "。<br />";
                        }

                        if (tmpDataSet.Tables[0].Rows[0]["td_zt"].ToString() != "5")
                        {
                            errMsg += "第" + (i + 1) + "条数据有误，原因是：该考生不是录取状态：" + element[0].ToString().Trim() + "。<br />";
                        }
                    }    
                    if (element[1].ToString().Trim() == "")
                        errMsg += "第" + (i + 1) + "条数据有误，原因是：该考生注销原因为空：" + element[0].ToString().Trim() + "。<br />";

                    if (tmpDataSet.Tables[1].Rows.Count > 0)
                    {
                        if (tmpDataSet.Tables[1].Rows[0]["types"].ToString() == "2")
                        {
                            sqlCmd = "  update  zk_zxlq set lqxx=@xxdm,zxyy=@zxyy,types=0 where ksh=@ksh";

                        }
                        else
                        {
                            errMsg += "第" + (i + 1) + "条数据有误，原因是：该报名号已提交注销：" + element[0].ToString().Trim() + "。<br />";
        ...
        public DataTable Select_zcxx(string ksh)
        {
            ...
            string sql = "select * from zk_zxlq where ksh=@ksh";
            List<SqlParameter> lisP = new List<SqlParameter>(){
        ...
        /// </summary>
        /// <returns></returns>
        public bool insert_zc(string ksh, string xxdm, string zxyy)
        {

            string sql = " insert into zk_zxlq (ksh,lqxx,zxyy,types) values (@ksh,@xxdm,@zxyy,0)";
            List<SqlParameter> lisP = new List<SqlParameter>(){
 			 new SqlParameter("@ksh",ksh), new SqlParameter("@xxdm",xxdm), new SqlParameter("@zxyy",zxyy)};
        ...
        /// </summary>
        /// <returns></returns>
        public bool up_zc(string ksh, string xxdm, string zxyy)
        {

            string sql = " update   zk_zxlq set lqxx=@xxdm,zxyy=@zxyy,types=0 where ksh=@ksh ";
            List<SqlParameter> lisP = new List<SqlParameter>(){
 			 new SqlParameter("@ksh",ksh), new SqlParameter("@xxdm",xxdm), new SqlParameter("@zxyy",zxyy)};
            _dbA.ExecuteNonQuery(sql, lisP, ref error, ref bReturn);
            if (bReturn)
        ...
        public bool del_zcxx(string ksh)
        {
            ...
            string sql = "delete zk_zxlq  where ksh in (" + ksh + ") ";
            _dbA.ExecuteNonQuery(sql, ref error, ref bReturn);
            if (bReturn)
            {
                return true;
        ...
        public bool update_zcxx(string lqxx, int type)
        {
            ...
            //考生轨迹
            string sqlgj = " insert into   zk_kslqgj (ksh,username,type,times)  (ksh,username,type,times)   select ksh,'" + SincciLogin.Sessionstu().UserName + "',0,getdate() from zk_zxlq   where lqxx='" + lqxx + "' and isnull(types,0)=0";
            _dbA.ExecuteNonQuery(sqlgj, ref error, ref bReturn);
            string sql = "update zk_zxlq set  types=" + type + " where lqxx='" + lqxx + "' and isnull(types,0)=0 ";
            _dbA.ExecuteNonQuery(sql, ref error, ref bReturn);
