...
{
    public partial class UserTypeManage : BPage
    { 
        #region "Page_Load"
        ...
        private void OnStart(QueryParam qp)
        {
            ...
            GridView1.DataSource = new Method().Sys_UserTypeList(qp, out RecordCount);
            GridView1.DataBind();
            //分页
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < this.GridView1.Rows.Count; i++)
            {
                ...
                if (cbox.Checked == true)
                {
                    ...
                    sat.TypeID = Convert.ToInt32(this.GridView1.Rows[i].Cells[2].Text);
                    sat.DataTable_Action_ = "Delete";

                    new Method().Sys_UserTypeInsertUpdateDelete(sat);
                    flag = true;
                    str = str + sat.TypeID + ",";
                }
            } string E_record = "删除: 用户类型管理数据：" + str + "";
            if (flag)
            {
