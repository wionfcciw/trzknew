...
{
    public partial class zkbm_Input : SPage //wf 2018-08-16 修改，单点登录统一控制 System.Web.UI.Page
    {
        public string ksh = "";
            ...
            if (Session["kaosheng"] != null)
            {
                if (BLL_Ks_Session.ksLogCheck())
                {
                    ...
                    if (checkinfo(item))
                    {
                        ...
                        {
                            Page.ClientScript.RegisterStartupScript(Page.GetType(), "", "<script>ymPrompt.alert({message:'保存成功',title:'提示'});setTimeout(function(){window.location.href='zkbm_ShowInfo.aspx';},1000);</script>");
                           
                        }
                        else
                        {
                            Page.ClientScript.RegisterStartupScript(Page.GetType(), "", "<script>ymPrompt.alert({message:'保存失败',title:'提示'});</script>");
       ...
       //     ddbj(xxdm);
        ...
        ///// 选择应届或往届
        ///// </summary> 
        //protected void ddlkslbdm_SelectedIndexChanged(object sender, EventArgs e)
        //{
        //    if (this.ddlkslbdm.SelectedValue == "2")
        //    {
        //        this.ddlbjdm.Enabled = false;
        //    }
        //    else
        //    {
        //        this.ddlbjdm.Enabled = true;
        //    }
        //} 
    }
}