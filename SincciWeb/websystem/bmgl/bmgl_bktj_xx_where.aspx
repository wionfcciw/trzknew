<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="bmgl_bktj_xx_where.aspx.cs" Inherits="SincciKC.websystem.bmgl.bmgl_bktj_xx_where" %>
...
.btn
{
    ...
    <script type="text/javascript">
        function opdg(where, xqdm, ID, Title, tiaoj, kaoci) {
           // alert(where + "__" + xqdm + "__" + ID);
            window.parent.addTab2(Title, '/websystem/bmgl/bmgl_bktj_bj_where.aspx?kaoci=' + kaoci + '&tiaojian=' + tiaoj + '&bmddm=' + ID + '&where=' + where + '&xqdm=' + xqdm + '&title=' + Title);
          
            return false;
        }
        function hideTest(Title, kaoci, where, xqdm) {
            window.parent.addTab2(Title, '/websystem/bmgl/bmgl_bktj_Print_where.aspx?xqdm=' + xqdm + '&kaoci=' + kaoci + '&where=' + where + '&type=2&title=' + Title);
            return false;
        }
        ...
        <HeaderTemplate>
         ...
         <ItemTemplate> 
                  ...
                  <%--  <td><%#Eval("xbdm").ToString()=="1"? "男":"女"%> </td>
                      ...
                      <td>  <a href="#" onclick='return opdg("<%=str %>","<%=xqdm %>","<%#Eval("bmddm")%>","班级报名分析","<%=gotj %>","<%=kaoci %>");'><%#Eval("bmdmc").ToString() == "合计" ? "" : "详情"%></a>
                      </td>                 
