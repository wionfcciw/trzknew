...
{
    public partial class SetBMDate : BPage
    {
        /// <summary>
        /// 设置报名时间控制类
        /// </summary>
        ...
        protected void btnSetDate_Click(object sender, EventArgs e)
        {
            ...
            {
                item.Xqdm = xqdm;
                item.Kssj = Convert.ToDateTime(BeTime);
                item.Jssj = Convert.ToDateTime(EnTime);

                if (config.DateTimeCompare(item.Kssj, item.Jssj) < 0)
                {

                    if (Bll_szbmsj.SelectDisp(xqdm))
                    {
                        Bll_szbmsj.update_zk_szbmsj_bm(item);
                        Response.Redirect("SetBMDate.aspx", false);
                    }
                    else
                    {
