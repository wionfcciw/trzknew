<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Sjzd_Manage.aspx.cs" Inherits="SincciKC.websystem.jcsj.Sjzd_Manage" %>
...
<link rel="stylesheet" type="text/css" href="../../easyui/themes/default/easyui.css" />
    ...
    <script type="text/javascript">
        ...
        function opdg2(ID, Title) {
            ymPrompt.win({ message: 'SjzdLb_Manage.aspx?ID=' + ID, width: 780, height: 500, title: Title, iframe: true, fixPosition: true, dragOut: false })
            return false;
        }

        function opdg(ID, Title) {
            ymPrompt.win({ message: 'Sjzd_AddEdit.aspx?zdlbdm=' + ID, width: 380, height: 250, title: Title, iframe: true, fixPosition: true, dragOut: false })
            return false;
        }
        function SelectAll() {
        ...
        <asp:Repeater ID="repDisplay" runat="server"  >
            ...
            <ItemTemplate>
                <tr style="height: 24px" class="datagrid-body" <%# (Container.ItemIndex+1) % 2 ==0?"bgcolor='#F7F7F7'":"" %>>
                   <td><input type="checkbox" name="CheckBox1" id="CheckBox1" value='<%# Eval("zdlbdm") %>'> </td>
                    <td> 
                       <%# (this.AspNetPager1.CurrentPageIndex - 1) * this.AspNetPager1.PageSize + Container.ItemIndex + 1%>
                       
                    </td>
                     <td>
                       <a href="#" class="btn" onclick="opdg2('<%# Eval("zdlbdm") %>', '<%# Eval("zdlbmc")%>')"><font color="#0000FF"> <%# Eval("zdlbdm")%> </font></a>
                    </td>
                    <td align="left">
                        <%# Eval("zdlbmc")%>
                    </td> 
                   <%-- <td>
                        <a href="#" onclick="return opdg('<%# Eval("zdlbdm") %>' ,'修改数据');">
