<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Jsxx.aspx.cs" Inherits="SincciKC.websystem.tyxm.Jsxx" %>
   ...
   <div id="tb" class="datagrid-toolbar"  >
         <table>
            ...
            OnClientClick="return MsgYes();" OnClick="btnDelete_Click" /> </td>
              <td><asp:Button ID="btndaoAll" runat="server" CssClass="icon-reload" Text="  导出数据" 
                      onclick="btndaoAll_Click"   /> </td>
                 <td style="width: 60px">
                     市(区)：
                 </td>
         ...
         <ItemTemplate> 
                   <tr style="height:24px" class="datagrid-body" <%# (Container.ItemIndex+1) % 2 ==0?"bgcolor='#F7F7F7'":"" %> >
                     ...
                     <td><asp:Label ID="lblksh" runat="server" Text='<%#Eval("ksh")%>'></asp:Label>  </td> 
                      ...
                      <td><%#Eval("bxmc")%> </td> 
                       <td>    <%#Eval("ksjsqr").ToString() == "2" ? "已确认" : Eval("ksjsqr").ToString() == "1" ? "已保存" : "未登录"%>
