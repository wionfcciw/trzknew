...
{
    public partial class SetBMDateTY : BPage
    {
        /// <summary>
        /// 设置报名时间控制类
        /// </summary>
        ...
        protected void btnSetDate_Click(object sender, EventArgs e)
        {
            ...
            {
                item.Xqdm = xqdm;
                item.Kssj_ty = Convert.ToDateTime(BeTime);
                item.Jssj_ty = Convert.ToDateTime(EnTime);

                if (config.DateTimeCompare(item.Kssj_ty, item.Jssj_ty) < 0)
                {

                    if (Bll_szbmsj.SelectDisp(xqdm))
                    {
                        Bll_szbmsj.update_zk_szbmsjTY(item);
                        Response.Redirect("SetBMDateTY.aspx", false);
                    }
                    else
                    {
