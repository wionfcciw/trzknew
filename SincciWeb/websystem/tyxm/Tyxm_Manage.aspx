<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Tyxm_Manage.aspx.cs" Inherits="SincciKC.websystem.tyxm.Tyxm_Manage" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        ...
        function opdg(ID, Title) {
            ymPrompt.win({ message: 'Tyxm_AddEdit.aspx?ID=' + ID, width: 400, height: 300, title: Title, iframe: true, fixPosition: true, dragOut: false })
            return false;
        }
        function SelectAll() {
...
<body>
   ...
   <%--         <asp:ListItem Value="1">必考项目</asp:ListItem>
            <asp:ListItem Value="2">抽定项目</asp:ListItem>--%>
            ...
            <ItemTemplate>
                <tr style="height: 24px" class="datagrid-body" <%# (Container.ItemIndex+1) % 2 ==0?"bgcolor='#F7F7F7'":"" %>>
                   <td><input type="checkbox" name="CheckBox1" id="CheckBox1" value='<%# Eval("id") %>'> </td>
                    ...
                    <td>
                        <%# Eval("lxId").ToString() == "1" ? "必考项目" : Eval("lxId").ToString() == "2" ? "抽定项目" : Eval("lxId").ToString() == "3" ? "自选项目1" : Eval("lxId").ToString() == "4" ? "自选项目2" : "自选项目3"%>
                    </td>
                    <td>
                        <%# Eval("name")%>
                    </td>
