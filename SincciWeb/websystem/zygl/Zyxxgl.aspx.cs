...
{
    public partial class Zyxxgl : BPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
               ...
               // BindGv();
                ...
                BLL_zk_zydz bll = new BLL_zk_zydz();
                ...
                {
                    ddlXpcInfo.SelectedValue = dpcdt.Rows[0]["dpcId"].ToString() + "1_1";
                }
            }
        }
        /// <summary>
        ...
        private void BindGv()
        {
            ...
            //DataTable dpcdt = new BLL_zk_zydz().Select_All_DpcIsPass("500");
            ////判断大批次的填报时间
            //for (int i = 0; i < dpcdt.Rows.Count; i++)
            //{
            //    if (!(config.DateTimeCompare(DateTime.Now, Convert.ToDateTime(dpcdt.Rows[i]["stime"])) > 0 && config.DateTimeCompare(DateTime.Now, Convert.ToDateTime(dpcdt.Rows[i]["etime"])) < 0))
            ...
            //if (dpcdt.Rows.Count>0)
            //{
            //    pcdm = dpcdt.Rows[0]["dpcDm"].ToString() + "1";
            //}
            ...
            pcdmstr = pcdm;
            this.Repeater1.DataSource = bllxxgl.ExecuteProcList_zy(where, pagesize, AspNetPager1.CurrentPageIndex, ref RecordCount, pcdm);
        ...
        private string strwhere()
        {
            ...
            if (KeyWord.Length > 0)
            {
                str = str + " (a.ksh='" + KeyWord + "' or a.xm like '%" + KeyWord + "%' or a.zkzh='" + KeyWord + "') and ";
            }
            if (tag.Length > 0)
            {
                if (Convert.ToInt32(tag) == 0)
                {
                    //str = str + "  a.ksh not in (select a.ksh from zk_ksxxgl a left join zk_kszyxx b on a.ksh=b.ksh where isnull(b.xxdm,'')<>'') and a.ksh not in (select a.ksh from zk_ksxxgl a left join zk_lqk b on a.ksh=b.ksh where isnull(td_zt,5)=5)  and ";
                    str = str + "  isnull(c.xxdm,'')=''  and isnull(td_zt,0)=0 and ";
                }
                ...
                {
                    str = str + " isnull(td_zt,0)=5 and ";
                }
                //else if (Convert.ToInt32(tag) == 3)
                //{
                //    str = str + "  isnull(zyksqr,0)='1' and ";
                //}
                ...
                //{
                //    str = str + "  isnull(zyksqr,0)='2' and isnull(zyxxdy,0)='0' and ";
                //}
            }
        ...
        protected void btndayin_Click(object sender, EventArgs e)
        {
            ...
            if (str.Length > 0)
            {
                str = str + "  ksqr=2 and zyksqr=2 and "; //只打已确认的考生。
            }
            if (str == "")
            {
        ...
        protected void btnResetTag_Click(object sender, EventArgs e)
        {
            ...
            if (str.Length > 0)
            {
                ...
                switch (UserType)
                {
                    ...
                    //学校用户 
                    case 4:
                        strset = " zyksqr=1,zyxxdy=0";
                        str = str + " and  zyksqr=2 and  isnull(zyxxqr,0)=0 ";
                        break;
                    //班级用户 
                    case 5:
                        strset = " 1<>1";
        ...
        protected void btndaoAll_Click(object sender, EventArgs e)
        {
            ...
            {
                DataTable tab = new BLL_zk_kszyxx().ExportData(" left(ksh,6)='14'+'" + xqtab.Rows[j]["xqdm"].ToString() + "' and xxdm!='' ");
                //  string sql = "";
        ...
        protected void btnyuans_Click(object sender, EventArgs e)
        {
            ...
            if (str.Length > 0)
            {
                ...
                switch (UserType)
                {
                    //系统管理员
                    case 1:
                        strset = " zyxqqr=0,zyksqr=1,zyxxqr=0,zyxxdy=0";
                        break;
                    //市招生办
                    case 2:
                        strset = " 1<>1"; break;
        ...
        protected void btnxxqr_Click(object sender, EventArgs e)
        {
            ...
            if (str.Length > 0)
            {
                str = str + "  ksqr=2 and zyksqr=2 and zyxxdy=1 and "; //只打已确认的考生。
            }
            if (str == "")
            {
        ...
        protected void btnwtb_Click(object sender, EventArgs e)
        {
            ...
            if (str.Length > 0)
            {
                ...
                switch (UserType)
                {
                    //系统管理员
                    case 1:
                        strset = " zyxqqr=0,zyksqr=0,zyxxqr=0,zyxxdy=0";
                        break;
                    //市招生办
                    case 2:
                        strset = " 1<>1"; break;
