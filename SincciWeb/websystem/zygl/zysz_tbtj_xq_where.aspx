<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="zysz_tbtj_xq_where.aspx.cs" Inherits="SincciKC.websystem.zysz.zysz_tbtj_xq_where" %>
...
.btn
{
    ...
    <script type="text/javascript">
        function opdg(where,ID, Title,tiaojian) {

            window.parent.addTab2(Title, '/websystem/zygl/zysz_tbtj_xx_where.aspx?tiaojian=' + tiaojian + '&xqdm=' + ID + '&where=' + where + '&title=' + Title);
            return false;
        }
    </script>
</head>
         ...
         <ItemTemplate> 
                  ...
                  <%--  <td><%#Eval("xbdm").ToString()=="1"? "男":"女"%> </td>
                    ...
                    <%-- <td><%#Eval("szbs")%> </td> 
                     ...
                     <td><%#Eval("Dldj")%> </td> 
                     <td><%#Eval("Swdj")%> </td> --%>
                      <td>  <a href="#" onclick='return opdg("<%=str%>","<%#Eval("xqdm")%>","学校志愿分析","<%=tiaojian%>");'><%#Eval("xqmc").ToString()=="合计"?"":"详情" %></a>
                      </td>                 
